...
bool CTestEffect::doActionSizeSafe(CActionSound &actionSound,bool prepareForUndo)
{
	...
	TAutoBuffer<float> input_kernel(M);
	for(sample_pos_t t=0;t<M;t++)
		input_kernel[t]=impulse[t];
	...
	float input_kernel[M];
	for(sample_pos_t i=0;i<M;i++) 
	{
		if(i!=M/2)
			input_kernel[i]=sin(2*M_PI*freq*(i-M/2))/(i-M/2)*(0.42-0.5*cos(2*M_PI*i/M)+0.08*cos(4*M_PI*i/M));
		else // avoid div by 0
			input_kernel[i]=2*M_PI*freq;
	}
#endif
	...
	input_kernel[M/2]+=1.0;
	*/

	TFFTConvolver<sample_t,float> c(input_kernel,M);
