...
Q_DECLARE_METATYPE(CSoundWindow*)

CSoundListWindow::CSoundListWindow(QWidget * parent, Qt::WindowFlags flags)
{
	setupUi(this);
...
void CSoundListWindow::rebuildSoundWindowList()
{
	...
	if(active!=NULL)
	{
		...
		{
			if(loadedSoundsList->topLevelItem(t)->data(0,Qt::UserRole).value<CSoundWindow *>()==active)
			{
...
void CSoundListWindow::switchToWindow(int index)
{
...
void CSoundListWindow::switchToWindow(CSoundWindow *window)
{
	if(!window)
		return;
	...
	{ // find the index in the sound list of the window and set it to be the current item
		if(loadedSoundsList->topLevelItem(index)->data(0,Qt::UserRole).value<CSoundWindow *>()==window)
		{
...
QSize CSoundListWindow::sizeHint() const
{
	return QSize(400,100);
}


void CSoundListWindow::on_loadedSoundsList_currentItemChanged(QTreeWidgetItem *currentItem,QTreeWidgetItem *previous)
{
	if(currentItem)
		gSoundFileManager->setActiveSoundWindow(currentItem->data(0,Qt::UserRole).value<CSoundWindow *>());
}
