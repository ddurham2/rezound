...
COggSaveParametersDialog::COggSaveParametersDialog(QWidget *mainWindow) :
	CActionParamDialog(mainWindow)
{
	...
	QWidget *p=newVertPanel(NULL);
		...
		p->layout()->addWidget(VBRGroupBox);
			minVBRCombo=addComboTextEntry(VBRGroupBox,N_("Minimum Bit Rate:"),br,CActionParamDialog::cpvtAsInteger,"");
			minVBRCombo->setIntegerValue(4);
			normalVBRCombo=addComboTextEntry(VBRGroupBox,N_("Normal Bit Rate:"),br,CActionParamDialog::cpvtAsInteger,"");
			normalVBRCombo->setIntegerValue(4);
			maxVBRCombo=addComboTextEntry(VBRGroupBox,N_("Maximum Bit Rate:"),br,CActionParamDialog::cpvtAsInteger,"");
...
void COggSaveParametersDialog::onGroupBoxToggled(bool on)
{
	if(!on)
...
bool COggSaveParametersDialog::show(AFrontendHooks::OggCompressionParameters &parameters)
{
