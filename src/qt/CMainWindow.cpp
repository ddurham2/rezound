...
void CMainWindow::backendInitialized()
{
	...
	*/
	...
	// add dock widgets
	{
		...
		w=m_transportWindow=new CTransportWindow(this);
		//s=w->size(); // 212x146
		//s.rheight()+=15;
		//s.rwidth()+=4;
		//dw->setFixedSize(s);
		...
		w=m_miscControlsWindow=new CMiscControlsWindow(this);
		//s=w->size(); // 212x146
		//s.rheight()+=15;
		//s.rwidth()+=4;
		//dw->setFixedSize(s);
		...
		w=m_soundListWindow=new CSoundListWindow();
		dw->setWidget(w);
		addDockWidget(Qt::TopDockWidgetArea, dw);
		...
		dw->setAllowedAreas(Qt::TopDockWidgetArea | Qt::BottomDockWidgetArea);
		w=m_levelMeters=new CPlaybackLevelMeters(0);
		dw->setWidget(w);
		...
		dw->setFeatures(dw->features()|QDockWidget::DockWidgetVerticalTitleBar);
		w=m_stereoPhaseMeters=new CStereoPhaseMeters(0);
		dw->setWidget(w);
		addDockWidget(Qt::BottomDockWidgetArea, dw);
		...
		dw->setFeatures(dw->features()|QDockWidget::DockWidgetVerticalTitleBar);
		w=m_frequencyAnalyzer=new CFrequencyAnalyzer(0);
		dw->setWidget(w);
		addDockWidget(Qt::BottomDockWidgetArea, dw);
#endif
...
*/
	...
	// add the meters
	{
		...
		m_levelMeters=new CPlaybackLevelMeters(0);
		layout=new QHBoxLayout;
		layout->setMargin(0);
		layout->addWidget(m_levelMeters);
		levelMetersFrame->setLayout(layout);

		m_stereoPhaseMeters=new CStereoPhaseMeters(0);
		stereoPhaseFrame->layout()->addWidget(m_stereoPhaseMeters);

		m_frequencyAnalyzer=new CFrequencyAnalyzer(0);
...
void CMainWindow::setSoundPlayerForMeters(ASoundPlayer *player)
{
...
void CMainWindow::restoreDockState() // called by CFrontendHooks
{
...
{
	/*
	CSoundWindow *wv=new CSoundWindow;
...
void CMainWindow::onViewMenu_aboutToShow()
{
...
void CMainWindow::switchToWindow(int index)
{
	m_soundListWindow->switchToWindow(index);
}

void CMainWindow::onRezAction_triggered()
{
	CRezAction *action=dynamic_cast<CRezAction *>(QObject::sender());
	if(!action)
		return;
	...
	{
		for(vector<CRezAction *>::iterator i=m_recentActions.begin();i!=m_recentActions.end();i++)
		{
...
#include "CReopenMenu.h"
#include "CRecentActionsMenu.h"

/* the usual case -- adding a CRezAction to the action registry */
void CMainWindow::registerAction(CRezAction *action)
{
...
void CMainWindow::buildActionMap() 
{
	...
	// File 
	registerAction(		new CRezAction(new CNewAudioFileActionFactory(new CNewAudioFileActionDialog(this)),		loadIcon("file_new.png")));
	registerAction(		new CRezAction(new COpenAudioFileActionFactory(new COpenAudioFileActionDialog(this)),		loadIcon("file_open.png")));
	registerAction(action=	new CRezAction(N_("&Reopen"),									loadIcon("file_open.png")));
		action->setMenu(new CReopenMenu);
	registerAction(		new CRezAction(new CSaveAudioFileActionFactory,							loadIcon("file_save.png")));
	registerAction(		new CRezAction(new CSaveAsAudioFileActionFactory(new CSaveAsAudioFileActionDialog(this)),	loadIcon("file_save_as.png")));
	registerAction(		new CRezAction(new CSaveSelectionAsActionFactory,						loadIcon("file_save_as.png")));
	registerAction(		new CRezAction(new CSaveAsMultipleFilesActionFactory(new CSaveAsMultipleFilesDialog(this)),	loadIcon("file_save_as.png")));
	registerAction(		new CRezAction(new CBurnToCDActionFactory(new CBurnToCDDialog(this)),				loadIcon("file_burn.png")));
	...
	registerAction(		new CRezAction(N_("&Close"),									loadIcon("file_close.png")));
	registerAction(		new CRezAction(N_("Re&vert"),									loadIcon("file_revert.png")));
	// -
	registerAction(		new CRezAction(new CRunMacroActionFactory(new CRunMacroDialog(this))				));
	registerAction(		new CRezAction(N_("Record Macro...")								));
	// -
	registerAction(		new CRezAction(N_("User No&tes..."),								loadIcon("notes.png")));
	// -
	registerAction(		new CRezAction(N_("&About ReZound"),								QIcon()	));
	registerAction(		new CRezAction(N_("&Setup Hotkeys..."),								QIcon() ));
	// -
	registerAction(		new CRezAction("- Just for testing",								QIcon() ));
	registerAction(		new CRezAction("Defrag",									QIcon() ));
	registerAction(		new CRezAction("PrintSAT",									QIcon() ));
	registerAction(		new CRezAction("VerifySAT",									QIcon() ));
	// -
	registerAction(		new CRezAction(N_("&Quit"),									loadIcon("exit.png")));

	// Control
	registerAction(		new CRezAction(m_transportWindow->recordButton));
	registerAction(		new CRezAction(m_transportWindow->playAllButton));
	registerAction(		new CRezAction(m_transportWindow->playAllLoopedButton));
	registerAction(		new CRezAction(m_transportWindow->playSelectionOnceButton));
	registerAction(		new CRezAction(m_transportWindow->playFromLeftEdgeOfScreenToEndButton));
	registerAction(		new CRezAction(m_transportWindow->playFromStartPositionButton));
	registerAction(		new CRezAction(m_transportWindow->playSelectionLoopedButton));
	registerAction(		new CRezAction(m_transportWindow->playSelectionLoopedSkipMostButton));
	registerAction(		new CRezAction(m_transportWindow->playSelectionLoopedGapBeforeRepeatButton));
	registerAction(		new CRezAction(m_transportWindow->stopButton));
	registerAction(		new CRezAction(m_transportWindow->pauseButton));
	// -
	registerAction(		new CRezAction(m_transportWindow->jumpToBeginningButton));
	registerAction(		new CRezAction(m_transportWindow->jumpToStartPositionButton));
	registerAction(		new CRezAction(m_transportWindow->jumpToPrevCueButton));
	registerAction(		new CRezAction(m_transportWindow->jumpToNextCueButton));

	registerAction(		m_transportWindow->m_shuttleBackwardAction=new CRezAction(N_("Shuttle Rewind"),			loadIcon("shuttle_backward.png")));
	registerAction(		m_transportWindow->m_shuttleAmountAction=new CRezAction(N_("Shuttle Amount"),			loadIcon("shuttle_normal.png")));
	registerAction(		m_transportWindow->m_shuttleForwardAction=new CRezAction(N_("Shuttle Forward"),			loadIcon("shuttle_forward.png")));


	// View Controls
	registerAction(		new CRezAction(N_("Zoom Out F&ull"),								loadIcon("zoom_out_full.png")));
	registerAction(		new CRezAction(N_("Zoom &Out"),									loadIcon("zoom_out.png")));
	registerAction(		new CRezAction(N_("Zoom &In"),									loadIcon("zoom_in.png")));
	registerAction(		new CRezAction(N_("Zoom &Fit Selection"),							loadIcon("zoom_fit.png")));
	// -
	registerAction(		new CRezAction(N_("Find &Start Position"),							loadIcon("normal_action_buff.png")));
	registerAction(		new CRezAction(N_("Find Sto&p Position"),							loadIcon("normal_action_buff.png")));
	// -
	registerAction(		new CRezAction(N_("&Redraw"),									loadIcon("normal_action_buff.png")));
	// -
	registerAction(		new CRezAction(N_("Switch to Previously Viewed File")						));
	registerAction(		new CRezAction(N_("Switch to Loaded File 1")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 2")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 3")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 4")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 5")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 6")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 7")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 8")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 9")							));
	registerAction(		new CRezAction(N_("Switch to Loaded File 10")							));


	// Edit
	registerAction(		new CRezAction(N_("Undo"),									loadIcon("edit_undo.png") ));
	registerAction(		new CRezAction(N_("Clear Undo History")								));
	// -
	registerAction(action=	new CRezAction(N_("&Recent Actions")								));
		action->setMenu(new CRecentActionsMenu(this));
	// -
	registerAction(		new CRezAction(new CCopyEditFactory(gChannelSelectDialog),					loadIcon("edit_copy.png") ));
	registerAction(		new CRezAction(new CCopyToNewEditFactory(gChannelSelectDialog),					loadIcon("edit_copy.png") ));
	registerAction(		new CRezAction(new CCutEditFactory(gChannelSelectDialog),					loadIcon("edit_cut.png") ));
	registerAction(		new CRezAction(new CCutToNewEditFactory(gChannelSelectDialog),					loadIcon("edit_cut.png")  ));
	registerAction(		new CRezAction(new CDeleteEditFactory(gChannelSelectDialog),					loadIcon("edit_delete.png") ));
	registerAction(		new CRezAction(new CCropEditFactory(gChannelSelectDialog),					loadIcon("edit_crop.png") ));
	// -
	registerAction(		new CRezAction(new CInsertPasteEditFactory(gPasteChannelsDialog),				loadIcon("edit_paste.png") ));
	registerAction(		new CRezAction(new CReplacePasteEditFactory(gPasteChannelsDialog),				loadIcon("edit_paste.png") ));
	registerAction(		new CRezAction(new COverwritePasteEditFactory(gPasteChannelsDialog),				loadIcon("edit_paste.png") ));
	registerAction(		new CRezAction(new CLimitedOverwritePasteEditFactory(gPasteChannelsDialog),			loadIcon("edit_paste.png") ));
	registerAction(		new CRezAction(new CMixPasteEditFactory(gPasteChannelsDialog),					loadIcon("edit_paste.png") ));
	registerAction(		new CRezAction(new CLimitedMixPasteEditFactory(gPasteChannelsDialog),				loadIcon("edit_paste.png") ));
	registerAction(		new CRezAction(new CFitMixPasteEditFactory(gPasteChannelsDialog),				loadIcon("edit_paste.png") ));
	registerAction(		new CRezAction(new CPasteAsNewEditFactory,							loadIcon("edit_paste.png") ));
	// -
	registerAction(		new CRezAction(new CInsertSilenceEditFactory(gChannelSelectDialog,new CInsertSilenceDialog(this))	));
	registerAction(		new CRezAction(new CMuteEditFactory(gChannelSelectDialog)						));
	// -
	registerAction(		new CRezAction(new CAddChannelsEditFactory(new CAddChannelsDialog(this))				));
	registerAction(		new CRezAction(new CDuplicateChannelEditFactory(new CDuplicateChannelDialog(this))			));
	registerAction(		new CRezAction(new CRemoveChannelsEditFactory(gChannelSelectDialog)					));
	registerAction(		new CRezAction(new CSwapChannelsEditFactory(new CSwapChannelsDialog(this))				));
	// -
	registerAction(		new CRezAction(new CRotateLeftEditFactory(gChannelSelectDialog,new CRotateDialog(this))			));
	registerAction(		new CRezAction(new CRotateRightEditFactory(gChannelSelectDialog,new CRotateDialog(this))		));
	// -
	registerAction(		new CRezAction(new CSelectionEditFactory(sSelectAll)							));
	registerAction(		new CRezAction(new CGrowOrSlideSelectionEditFactory(new CGrowOrSlideSelectionDialog(this))		));
	registerAction(		new CRezAction(new CSelectionEditFactory(sSelectToBeginning)						));
	registerAction(		new CRezAction(new CSelectionEditFactory(sSelectToEnd)							));
	registerAction(		new CRezAction(new CSelectionEditFactory(sFlopToBeginning)						));
	registerAction(		new CRezAction(new CSelectionEditFactory(sFlopToEnd)							));
	registerAction(		new CRezAction(new CSelectionEditFactory(sSelectToSelectStart)						));
	registerAction(		new CRezAction(new CSelectionEditFactory(sSelectToSelectStop)						));
	// -
	registerAction(		new CRezAction(new CAddCueWhilePlayingActionFactory							));


	// Effects
	registerAction(		new CRezAction(new CReverseEffectFactory(gChannelSelectDialog) 							));
	// -
	registerAction(		new CRezAction(new CChangeVolumeEffectFactory(gChannelSelectDialog,new CNormalVolumeChangeDialog(this))		));
	registerAction(		new CRezAction(new CSimpleGainEffectFactory(gChannelSelectDialog,new CNormalGainDialog(this)) 			));
	registerAction(		new CRezAction(new CCurvedGainEffectFactory(gChannelSelectDialog,new CAdvancedGainDialog(this)) 		));
	// -
	registerAction(		new CRezAction(new CSimpleChangeRateEffectFactory(gChannelSelectDialog,new CNormalRateChangeDialog(this))	));
	registerAction(		new CRezAction(new CCurvedChangeRateEffectFactory(gChannelSelectDialog,new CAdvancedRateChangeDialog(this))	));
	// -
	registerAction(		new CRezAction(new CFlangeEffectFactory(gChannelSelectDialog,new CFlangeDialog(this)) 				));
	registerAction(		new CRezAction(new CSimpleDelayEffectFactory(gChannelSelectDialog,new CSimpleDelayDialog(this)) 		));
	registerAction(		new CRezAction(new CQuantizeEffectFactory(gChannelSelectDialog,new CQuantizeDialog(this)) 			));
	registerAction(		new CRezAction(new CDistortionEffectFactory(gChannelSelectDialog,new CDistortionDialog(this)) 			));
	registerAction(		new CRezAction(new CVariedRepeatEffectFactory(gChannelSelectDialog,new CVariedRepeatDialog(this)) 		));

	registerAction(		new CRezAction(new CTestEffectFactory(gChannelSelectDialog)							));


	// Filter
	registerAction(		new CRezAction(new CConvolutionFilterFactory(gChannelSelectDialog,new CConvolutionFilterDialog(this))			));
	registerAction(		new CRezAction(new CArbitraryFIRFilterFactory(gChannelSelectDialog,new CArbitraryFIRFilterDialog(this)),		loadIcon("filter_custom.png") ));
	registerAction(		new CRezAction(new CMorphingArbitraryFIRFilterFactory(gChannelSelectDialog,new CMorphingArbitraryFIRFilterDialog(this)),loadIcon("filter_custom.png") ));
	// -
	registerAction(		new CRezAction(new CSinglePoleLowpassFilterFactory(gChannelSelectDialog,new CSinglePoleLowpassFilterDialog(this)),	loadIcon("filter_lowpass.png") ));
	registerAction(		new CRezAction(new CSinglePoleHighpassFilterFactory(gChannelSelectDialog,new CSinglePoleHighpassFilterDialog(this)),	loadIcon("filter_highpass.png") ));
	registerAction(		new CRezAction(new CBandpassFilterFactory(gChannelSelectDialog,new CBandpassFilterDialog(this)),			loadIcon("filter_bandpass.png") ));
	registerAction(		new CRezAction(new CNotchFilterFactory(gChannelSelectDialog,new CNotchFilterDialog(this)),				loadIcon("filter_notch.png") ));
	// -
	registerAction(		new CRezAction(new CBiquadResLowpassFilterFactory(gChannelSelectDialog,new CBiquadResLowpassFilterDialog(this)),	loadIcon("filter_lowpass.png") ));
	registerAction(		new CRezAction(new CBiquadResHighpassFilterFactory(gChannelSelectDialog,new CBiquadResHighpassFilterDialog(this)),	loadIcon("filter_highpass.png") ));
	registerAction(		new CRezAction(new CBiquadResBandpassFilterFactory(gChannelSelectDialog,new CBiquadResBandpassFilterDialog(this)),	loadIcon("filter_bandpass.png") ));


	// Looping
	registerAction(		new CRezAction(new CMakeSymetricActionFactory(gChannelSelectDialog) 						));
	// -
	registerAction(		new CRezAction(new CAddNCuesActionFactory(new CAddNCuesDialog(this)) 						));
	registerAction(		new CRezAction(new CAddTimedCuesActionFactory(new CAddTimedCuesDialog(this)) 					));


	// Remaster
	registerAction(		new CRezAction(new CSimpleBalanceActionFactory(NULL,new CSimpleBalanceActionDialog(this)) 			));
	registerAction(		new CRezAction(new CCurvedBalanceActionFactory(NULL,new CCurvedBalanceActionDialog(this)) 			));
	// -
	registerAction(		new CRezAction(new CMonoizeActionFactory(NULL,new CMonoizeActionDialog(this)) 					));
	// -
	registerAction(		new CRezAction(new CNoiseGateActionFactory(gChannelSelectDialog,new CNoiseGateDialog(this)) 			));
	registerAction(		new CRezAction(new CCompressorActionFactory(gChannelSelectDialog,new CCompressorDialog(this)) 			));
	registerAction(		new CRezAction(new CNormalizeActionFactory(gChannelSelectDialog,new CNormalizeDialog(this)) 			));
	registerAction(		new CRezAction(new CAdaptiveNormalizeActionFactory(gChannelSelectDialog,new CAdaptiveNormalizeDialog(this)) 	));
	registerAction(		new CRezAction(new CMarkQuietAreasActionFactory(new CMarkQuietAreasDialog(this)) 				));
	registerAction(		new CRezAction(new CShortenQuietAreasActionFactory(new CShortenQuietAreasDialog(this)) 				));
	// -
	registerAction(		new CRezAction(new CResampleActionFactory(gChannelSelectDialog,new CResampleDialog(this)) 			));
	registerAction(		new CRezAction(new CChangePitchActionFactory(gChannelSelectDialog,new CChangePitchDialog(this)) 		));
	registerAction(		new CRezAction(new CChangeTempoActionFactory(gChannelSelectDialog,new CChangeTempoDialog(this)) 		));
	// -
	registerAction(		new CRezAction(new CRemoveDCActionFactory(gChannelSelectDialog) 						));
	registerAction(		new CRezAction(new CInvertPhaseActionFactory(gChannelSelectDialog) 						));
	// -
	registerAction(		new CRezAction(new CUnclipActionFactory(gChannelSelectDialog) 							));

	// Generate
	registerAction(		new CRezAction(new CGenerateNoiseActionFactory(gChannelSelectDialog,new CGenerateNoiseDialog(this)) 		));
	registerAction(		new CRezAction(new CGenerateToneActionFactory(gChannelSelectDialog,new CGenerateToneDialog(this)) 		));
	...
	for(map<const string,CRezAction *>::iterator i=actionRegistry.begin();i!=actionRegistry.end();i++)
	{
		if(dynamic_cast<CRezAction *>(i->second)!=NULL)
			gRegisteredActionFactories[i->first]=((CRezAction *)i->second)->getActionFactory();
	}
...
void CMainWindow::buildMenu(QMenu *menu,const CNestedDataFile *menuLayoutFile,const string menuKey,const string actionName)
{
	if(actionName=="-") 
	{
	...
	{	// add as normal menu item
		...
		if(actionRegistry.find(strippedItemName)!=actionRegistry.end())
		{
			...
			CRezAction *action=actionRegistry[strippedItemName];
...
void CMainWindow::createMenus()
{
	...
	for(map<const string,CRezAction *>::iterator i=actionRegistry.begin();i!=actionRegistry.end();i++)
	{
		if(i->second->parent()==this) // parent is still CMainWindow.. so buildMenu() didn't find anywhere to put it
...
void CMainWindow::buildLADSPAMenus()
{
	...
	else
	{
		...
		if(LADSPAActionFactories.size()>20)
		{
			...
			map<CLADSPAActionFactory *,CRezAction *> actionByFactory;
			for(map<const char,multimap<const string,CLADSPAActionFactory *> >::iterator i=nameGrouped.begin();i!=nameGrouped.end();i++)
			{
				...
				{
					CRezAction *a=new CRezAction(t->second);
					a->setParent(this);
					submenu->addAction(a);
					registerAction(a);
		...
		else
		{
			for(size_t t=0;t<LADSPAActionFactories.size();t++)
			{
				CRezAction *a=new CRezAction(LADSPAActionFactories[t]);
				a->setParent(this);
				menu->addAction(a);
				registerAction(a);
			}
...
void CMainWindow::setupKeyBindings()
{
	...
	// since we'll be handling these key event's specially in CTransportWindow, make them have a shortcut context of their parent widget (menubar in this case)
