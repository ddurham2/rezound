...
class QCheckBox;
class CLoadedSound;
class CSoundWindowScrollArea;
class CSamplePosSpinBox;
class CWaveRuler;
class CAddCueActionFactory;
class CSoundWindowScrollArea;

// IWaveDrawing is an interface used by CSoundWindow and CGraphParamValue both borrowing needing a couple of methods in common for calling on drawWaveRuler()
...
#include "ui_CSoundWindow.h"
class CSoundWindow : public IWaveDrawing, private Ui::CSoundWindow
{
	Q_OBJECT

public:
	CSoundWindow(QLayout * parent, CLoadedSound *loaded);
	virtual ~CSoundWindow();

	// this brings this CSoundWindow object to the front
...
private:
	friend class CSoundWindowScrollArea;
