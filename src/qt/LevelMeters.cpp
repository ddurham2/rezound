...
public:
    ...
    LevelMeter(QWidget *parent,int *_pLevelMeterTickCount) :
		QWidget(parent),
...
class BalanceMeter : public QWidget
{
public:
    BalanceMeter(QWidget *parent,LevelMeter *_left,LevelMeter *_right) :
		QWidget(parent),
		left(_left),
		right(_right)//,
		//m_RMSBalance(0),
...
class LabelLayout : public QLayout
{
public:
    LabelLayout(QWidget *parent)
		: QLayout(parent)
	{
...
LevelMeters::LevelMeters(QWidget *parent) :
	QWidget(parent),
...
bool LevelMeters::eventFilter(QObject *watched, QEvent *event)
{
...
void LevelMeters::resetGrandMaxPeakLevels()
{
	for(size_t t=0;t<m_levelMeters.size();t++)
		m_levelMeters[t]->setGrandMaxPeakLevel(0);
}

void LevelMeters::initialize(unsigned channelCount,bool justCleanup)
{
	// delete all meters
	qDeleteAll(children());
	m_levelMeters.clear();
	...
	for(unsigned t=0;t<channelCount;t++)
	{
        ...
        LevelMeter *lm=new LevelMeter(frame,&levelMeterTickCount);
		...
		if((t%2)==1)
		{
            ...
            BalanceMeter *bm=new BalanceMeter(c,m_levelMeters[t-1],m_levelMeters[t]);
...
void LevelMeters::timerEvent(QTimerEvent *e)
{
...
/*
void LevelMeters::resizeEvent(QResizeEvent *e)
{
...
*/
/*
void LevelMeters::setGeometry(const QRect &r)
{
printf("hello from LevelMeters::setGeometry\n");
	QWidget::setGeometry(r);
	recreateLevelLabels();
}
*/
...
PlaybackLevelMeters::PlaybackLevelMeters(QWidget *parent) :
    LevelMeters(parent),
	m_soundPlayer(0)
{
}

PlaybackLevelMeters::~PlaybackLevelMeters()
{
}

void PlaybackLevelMeters::setSoundPlayer(ASoundPlayer *soundPlayer)
{
...
void PlaybackLevelMeters::updateMeters()
{
	if(
		m_soundPlayer && 
...
RecordLevelMeters::RecordLevelMeters(QWidget *parent) :
    LevelMeters(parent),
	m_soundRecorder(0)
{
}

RecordLevelMeters::~RecordLevelMeters()
{
}

void RecordLevelMeters::setSoundRecorder(ASoundRecorder *soundRecorder)
{
...
void RecordLevelMeters::updateMeters()
{
	if(
		m_soundRecorder /*&& 
