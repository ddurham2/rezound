...
class SoundWindowScrollArea : public QAbstractScrollArea
{
	Q_OBJECT 

public:
	SoundWindowScrollArea(SoundWindow *_wv) :
		QAbstractScrollArea(),
		wv(_wv),
		first(true),
...
protected:
	...
	void resizeEvent(QResizeEvent *e)
	{
		...
		wv->setHorzZoom(wv->getHorzZoom(),SoundWindow::hrtLeftEdge,true);
...
class WaveRuler : public QWidget
{
	Q_OBJECT


public:
	WaveRuler(SoundWindow *_wv) :
		QWidget(),
		wv(_wv)
	{
		rulerFont=font();
			...
			if(draggingSelectionToo)
			{
				...
				{	// move start position with cue drag
					...
					{ // dragged cue past stop position 
						...
						wv->updateFromSelectionChange(SoundWindow::lcpStop);
					}
					else
					{
						wv->loadedSound->channel->setStartPosition(newTime);
						wv->updateFromSelectionChange(SoundWindow::lcpStart);
					}
...
public:
	CSamplePosSpinBox(SoundWindow *_sw) :
		sw(_sw)
	{
...
#include "SoundWindow.moc"

static void playTrigger(void *Pthis);
SoundWindow::SoundWindow(QLayout * parent,CLoadedSound *_loadedSound) :
	...
	pausedLEDOn(false)
{
	...
	// add scroll area and ruler
	{
		...
		wvsa=new SoundWindowScrollArea(this);
...
void SoundWindow::onAfterLayout()
{
	setVertZoom(0.0,true);
	showAmount(gInitialLengthToShow,0);
}

void SoundWindow::setActiveState(bool _isActive)
{
	isActive=_isActive;
	if(isActive)
	{
		static_cast<MainWindow *>(parent())->positionShuttleGivenSpeed(loadedSound->channel->getSeekSpeed(),shuttleControlScalar,shuttleControlSpringBack);
...
bool SoundWindow::getActiveState() const
{
	return isActive;
}

void SoundWindow::updateFromEdit(bool undoing)
{
	if(undoing)
...
void SoundWindow::recreateMuteButtons()
{
...
void SoundWindow::on_invertMutesButton_clicked()
{
	for(unsigned t=0;t<muteButtonCount;t++)
...
int SoundWindow::getZoomDecimalPlaces() const
{
...
const string SoundWindow::getLengthString(sample_pos_t length,TimeUnits units)
{
	if(units==tuSeconds)
...
void SoundWindow::updateAllStatusInfo()
{
...
void SoundWindow::updateSelectionStatusInfo()
{
	for(size_t t=(TimeUnits)0;t<TIME_UNITS_COUNT;t++)
	{
...
void SoundWindow::updateZoomStatusInfo()
{
	// horz
...
{
	SoundWindow *that=(SoundWindow *)Pthis;
...
void SoundWindow::updatePlayPositionStatusInfo()
{
	/*
...
void SoundWindow::on_infoFrame_customContextMenuRequested(const QPoint &pos)
{
...
void SoundWindow::setHorzZoom(double v,HorzRecenterTypes horzRecenterType,bool force)
{
...
const sample_pos_t SoundWindow::getHorzSize() const
{
...
const double SoundWindow::getHorzZoom() const
{
	return lastHorzZoom;
}

void SoundWindow::horzZoomSelectionFit()
{
...
void SoundWindow::redraw()
{
	wvsa->viewport()->repaint(); // ??? try using update
	ruler->update();
}

const float SoundWindow::getVertZoom() const
{
	return lastVertZoom;
}

void SoundWindow::setVertZoom(float v,bool force)
{
...
const int SoundWindow::getVertSize() const
{
	return (int)ceil(MAX_WAVE_HEIGHT/vertZoomFactor);
}


void SoundWindow::setHorzOffset(sample_pos_t v)
{
	// ??? have a problem in that QScrollBars take 32bit ints.. 
	wvsa->horizontalScrollBar()->setValue(v); 
}

const sample_pos_t SoundWindow::getHorzOffset() const
{
	return horzOffset;
}

void SoundWindow::setVertOffset(int v)
{
	if(v!=vertOffset)
	{
		vertOffset=v;
...
const int SoundWindow::getVertOffset() const
{
	return vertOffset;
}

const sample_pos_t SoundWindow::getHorzOffsetToCenterTime(sample_pos_t time) const
{
...
const sample_pos_t SoundWindow::getHorzOffsetToCenterStartPos() const
{
	return getHorzOffsetToCenterTime(loadedSound->channel->getStartPosition());
}

const sample_pos_t SoundWindow::getHorzOffsetToCenterStopPos() const
{
	return getHorzOffsetToCenterTime(loadedSound->channel->getStopPosition());
}

void SoundWindow::showAmount(double seconds,sample_pos_t pos,int marginPixels)
{
...
const sample_pos_t SoundWindow::snapPositionToCue(sample_pos_t p) const
{
...
const int SoundWindow::getCueScreenX(size_t cueIndex) const
{
...
const sample_pos_t SoundWindow::getSamplePosForScreenX(int X) const
{
...
void SoundWindow::setSelectStartFromScreen(sample_pos_t X)
{
...
void SoundWindow::setSelectStopFromScreen(sample_pos_t X)
{
...
void SoundWindow::updateFromSelectionChange(LastChangedPositions _lastChangedPosition)
{
...
void SoundWindow::onAutoScroll()
{
	ruler->onAutoScroll();
}

void SoundWindow::drawPlayPosition(sample_pos_t dataPosition,bool justErasing,bool scrollToMakeVisible)
{
	int drawPlayStatusX;
...
void SoundWindow::on_horzZoomSlider_valueChanged()
{
	...
	if(QApplication::mouseButtons()&Qt::LeftButton)
	{
		...
		if(QApplication::keyboardModifiers()&Qt::ShiftModifier)
			horzRecenterType=SoundWindow::hrtStart;
		else if(QApplication::keyboardModifiers()&Qt::ControlModifier)
			horzRecenterType=SoundWindow::hrtStop;
	}
...
#if 0
void SoundWindow::updateHorzZoomDial()
{
...
void SoundWindow::on_vertZoomSlider_valueChanged()
{
...
/*
void SoundWindow::updateVertZoomDial()
{
				// inverse of expr in SoundWindow::on_vertZoomSlider_valueChanged()
...
const map<string,string> SoundWindow::getPositionalInfo() const
{
...
void SoundWindow::setPositionalInfo(const map<string,string> _info)
{
