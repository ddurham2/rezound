...
CDiskEntityParamValue::CDiskEntityParamValue(const char *_name,const string initialEntityName,DiskEntityTypes _entityType) :
	name(_name),
...
CDiskEntityParamValue::~CDiskEntityParamValue()
{
}

void CDiskEntityParamValue::on_browseButton_clicked()
{
	switch(entityType)
	{
...
const string CDiskEntityParamValue::getEntityName() const
{
	return decodeFilenamePresetParameter(entityNameTextBox->text().toStdString());
}

const bool CDiskEntityParamValue::getOpenAsRaw() const
{
...
void CDiskEntityParamValue::setEntityName(const string entityName,bool openAsRaw)
{
...
const string CDiskEntityParamValue::getName() const
{
	return name;
}

void CDiskEntityParamValue::setTipText(const string &text)
{
	label->setToolTip(text.c_str());	
	entityNameTextBox->setToolTip(text.c_str());
}

string CDiskEntityParamValue::getTipText() const
{
	return label->toolTip().toStdString();
}

const CDiskEntityParamValue::DiskEntityTypes CDiskEntityParamValue::getEntityType() const
{
	return entityType;
}

void CDiskEntityParamValue::readFromFile(const string &prefix,CNestedDataFile *f)
{
...
void CDiskEntityParamValue::writeToFile(const string &prefix,CNestedDataFile *f)
{
	const string key=prefix DOT getName();
