...
CGraphParamValue::CGraphParamValue(const char *_name) :
	name(_name),
...
CGraphParamValue::~CGraphParamValue()
{
}

void CGraphParamValue::setSound(CSound *_sound,sample_pos_t _start,sample_pos_t _stop)
{
	sound=_sound;
	start=_start;
	stop=_stop;
...
void CGraphParamValue::setHorzParameters(const string horzAxisLabel,const string horzUnits,AActionParamMapper *_horzValueMapper)
{
...
void CGraphParamValue::setVertParameters(const string vertAxisLabel,const string vertUnits,AActionParamMapper *_vertValueMapper)
{
...
void CGraphParamValue::clearNodes()
{
	nodes.clear();
...
void CGraphParamValue::on_scalarSpinner_valueChanged(int v)
{
	setScalar(v);
	pushUndo();
}
...
void CGraphParamValue::on_clearButton_clicked()
{
	horzDeformSlider->setValue(0);
	vertDeformSlider->setValue(0);
	clearNodes();
...
void CGraphParamValue::on_horzFlipButton_clicked()
{
...
void CGraphParamValue::on_vertFlipButton_clicked()
{
...
void CGraphParamValue::on_smoothButton_clicked()
{
...
void CGraphParamValue::on_undoButton_clicked()
{
	popUndo();
}

void CGraphParamValue::on_redoButton_clicked()
{
	popRedo();
}
...
void CGraphParamValue::on_vertDeformSlider_sliderReleased()
{
	pushUndo();
}

void CGraphParamValue::on_horzDeformSlider_sliderReleased()
{
	pushUndo();
}

const int CGraphParamValue::getCueScreenX(size_t cueIndex) const
{
...
const sample_pos_t CGraphParamValue::getSamplePosForScreenX(int X) const
{
...
bool CGraphParamValue::eventFilter(QObject *w,QEvent *e)
{
...
void CGraphParamValue::drawHorzRuler(QPaintEvent *ev)
{
...
void CGraphParamValue::onGraphCanvasPaint(QPaintEvent *e)
{
...
void CGraphParamValue::drawVertRuler(QPaintEvent *ev)
{
...
void CGraphParamValue::onGraphCanvasResize(QResizeEvent *e)
{
...
int CGraphParamValue::getGraphPanelWidth() const
{
...
// always returns an odd value >=1
int CGraphParamValue::getGraphPanelHeight() const
{
...
int CGraphParamValue::insertIntoNodes(const CGraphParamValueNode &node)
{
	// sanity checks
	if(nodes.size()<2)
...
void CGraphParamValue::onCreateOrStartDragNode(QMouseEvent *ev)
{
...
void CGraphParamValue::onDragNode(QMouseEvent *ev)
{
	if(draggingNode!=-1)
	{
...
void CGraphParamValue::onStopDragNode(QMouseEvent *e)
{
	draggingNode=-1;
	clearStatus();
}

void CGraphParamValue::onDestroyNode(QMouseEvent *ev)
{
	int nodeIndex=findNodeAt(ev->x(),ev->y());
...
int CGraphParamValue::findNodeAt(int x,int y)
{
	for(size_t t=0;t<nodes.size();t++)
	{
		CGraphParamValueNode &n=nodes[t];
...
double CGraphParamValue::deformNodeX(double x,int sliderValue) const
{
/*
...
double CGraphParamValue::undeformNodeX(double x,int sliderValue) const
{
...
int CGraphParamValue::nodeToScreenX(const CGraphParamValueNode &node)
{
	return (int)(deformNodeX(node.x,horzDeformSlider->value())*(graphCanvas->width()-1));
}

int CGraphParamValue::nodeToScreenY(const CGraphParamValueNode &node)
{
	return (int)((1.0-deformNodeX(node.y,vertDeformSlider->value()))*(getGraphPanelHeight()));
}

double CGraphParamValue::screenToNodeHorzValue(int x,bool undeform)
{
...
double CGraphParamValue::screenToNodeVertValue(int y,bool undeform)
{
...
void CGraphParamValue::updateNumbers()
{
	horzRuler->update();
	vertRuler->update();
	updateStatus();
...
const CGraphParamValueNodeList &CGraphParamValue::getNodes() const
{
	retNodes=nodes;
 ...
 */
void CGraphParamValue::copyFrom(const CGraphParamValue *src)
{
...
void CGraphParamValue::swapWith(CGraphParamValue *src)
{
	int t;
...
const string CGraphParamValue::getVertValueString(double vertValue) const
{
	return istring(vertValueMapper->interpretValue(vertValue),5,3);
}

const string CGraphParamValue::getHorzValueString(double horzValue) const
{
	if(sound==NULL)
...
void CGraphParamValue::updateStatus()
{
...
void CGraphParamValue::clearStatus()
{
...
const int CGraphParamValue::getScalar() const
{
	return scalarSpinner->isHidden() ? vertValueMapper->getDefaultScalar() : vertValueMapper->getScalar();
}

void CGraphParamValue::setScalar(const int scalar)
{
	prevScalar=scalar;
...
const int CGraphParamValue::getMinScalar() const
{
	/*
	int lo=0,hi=0;
...
const int CGraphParamValue::getMaxScalar() const
{
	/*
	int lo=0,hi=0;
	if(scalarSpinner!=NULL)
...
const string CGraphParamValue::getName() const
{
	return name;
}

CGraphParamValue::RState::RState(CGraphParamValue *v)
{
	nodes=v->nodes;
	scalar=v->prevScalar;
...
void CGraphParamValue::pushUndo()
{
...
void CGraphParamValue::applyState(RState *s)
{
	nodes=s->nodes;
	setScalar(s->scalar);
...
void CGraphParamValue::clearUndo()
{
...
void CGraphParamValue::clearRedo()
{
...
void CGraphParamValue::setUndoButtonsState()
{
...
void CGraphParamValue::readFromFile(const string &prefix,CNestedDataFile *f)
{
	pushUndo();
...
void CGraphParamValue::writeToFile(const string &prefix,CNestedDataFile *f) const
{
	const string key=prefix DOT getName();
