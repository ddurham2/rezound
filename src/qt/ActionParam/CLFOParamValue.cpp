...
CLFOParamValue::CLFOParamValue(const char *_name,const string ampUnits,const string ampTitle,const double maxAmp,const string freqUnits,const double maxFreq,const bool hideBipolarLFOs) :
	name(_name),

	amplitudeSlider(new CConstantParamValue(
		new CActionParamMapper_linear(1.0,min((int)maxAmp,1),min((int)maxAmp,1),(int)maxAmp),
		false,
		gettext(ampTitle.c_str())
	)),

	frequencySlider(new CConstantParamValue(
		new CActionParamMapper_linear(1.0,min((int)maxFreq,1),min((int)maxFreq,1),(int)maxFreq),
		false,
		N_("Frequency")
	)),

	phaseSlider(new CConstantParamValue(
...
CLFOParamValue::~CLFOParamValue()
{
}

void CLFOParamValue::setMinSize(int _minWidth,int _minHeight)
{
	setMinimumSize(_minWidth,_minHeight);
}

void CLFOParamValue::on_LFOTypeComboBox_currentIndexChanged(int index)
{
...
const CLFODescription CLFOParamValue::getValue()
{
	//validateRange();
...
const string CLFOParamValue::getName() const
{
	return name;
}

/*
void CLFOParamValue::setTipText(const FXString &text)
{
...
QString CLFOParamValue::getTipText() const
{
	return titleLabel->getTipText();
}
*/

void CLFOParamValue::readFromFile(const string &prefix,CNestedDataFile *f)
{
...
void CLFOParamValue::writeToFile(const string &prefix,CNestedDataFile *f)
{
	const string key=prefix DOT getName();
