...
ComboTextParamValue::ComboTextParamValue(const char *_name,const vector<string> &items,bool _isEditable) :
	asString(false),
...
ComboTextParamValue::~ComboTextParamValue()
{
}

void ComboTextParamValue::on_comboBox_editTextChanged(const QString &text)
{
	Q_EMIT changed();
}

void ComboTextParamValue::on_comboBox_currentIndexChanged(const QString &text)
{
	Q_EMIT changed();
}

const int ComboTextParamValue::getIntegerValue()
{
	if(isEditable)
...
const string ComboTextParamValue::getStringValue()
{
	return comboBox->currentText().toStdString();
}

void ComboTextParamValue::setIntegerValue(const int value)
{
	if(isEditable)
...
void ComboTextParamValue::setItems(const vector<string> &items)
{
	this->items=items;
...
const vector<string> ComboTextParamValue::getItems() const
{
	/*
	vector<string> items;
...
/*
void ComboTextParamValue::setIcons(const vector<FXIcon *> icons)
{
...
void ComboTextParamValue::setCurrentItem(const int current)
{
	comboBox->setCurrentIndex(current);
}

const string ComboTextParamValue::getName() const
{
	return name;
}

void ComboTextParamValue::setTipText(const string &text)
{
	label->setToolTip(text.c_str());
	comboBox->setToolTip(text.c_str());
}

string ComboTextParamValue::getTipText() const
{
	return label->toolTip().toStdString();
}

void ComboTextParamValue::enable()
{
	setEnabled(true);
}

void ComboTextParamValue::disable()
{
	setEnabled(false);
}

void ComboTextParamValue::readFromFile(const string &prefix,CNestedDataFile *f)
{
	if(asString)
	{
...
void ComboTextParamValue::writeToFile(const string &prefix,CNestedDataFile *f)
{
