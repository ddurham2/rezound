...
class CStereoPhaseMeter : public QWidget
{
public:
	CStereoPhaseMeter(sample_t *_samplingBuffer,size_t _samplingNFrames,unsigned _samplingNChannels,unsigned _samplingLeftChannel,unsigned _samplingRightChannel) :
		QWidget(),
		qimage(NULL),
...
private:
	...
	#if 0  // I think this could be defined in the CStereoPhaseMeters window instead of in each stereo meter
...
CStereoPhaseMeters::CStereoPhaseMeters(QWidget *parent) :
	QWidget(parent),
	m_timerId(-1),
	m_soundPlayer(NULL)
{
...
CStereoPhaseMeters::~CStereoPhaseMeters()
{
}

void CStereoPhaseMeters::setSoundPlayer(ASoundPlayer *soundPlayer)
{
	...
	for(unsigned t=0;t<soundPlayer->devices[0].channelCount/2;t++)
	{
		...
		CStereoPhaseMeter *spm=new CStereoPhaseMeter(
...
bool CStereoPhaseMeters::eventFilter(QObject *watched, QEvent *ev)
{
...
void CStereoPhaseMeters::timerEvent(QTimerEvent *e)
{
	if(
		e->timerId()==m_timerId && 
		m_soundPlayer!=NULL && 
...
//void CStereoPhaseMeters::resizeEvent()

void CStereoPhaseMeters::onZoomDial(int val)
{
	for(size_t t=0;t<m_stereoPhaseMeters.size();t++)
