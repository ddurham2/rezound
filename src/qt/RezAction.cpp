...
/*
FXDEFMAP(RezAction) RezActionMap[]=
{
	//Message_Type				ID				Message_Handler

	FXMAPFUNC(SEL_LEFTBUTTONRELEASE,	0,				RezAction::onMouseClick),
	FXMAPFUNC(SEL_RIGHTBUTTONRELEASE,	0,				RezAction::onMouseClick),

	FXMAPFUNC(SEL_KEYRELEASE,		0,				RezAction::onKeyClick),

	FXMAPFUNC(SEL_COMMAND,			RezAction::ID_HOTKEY,	RezAction::onCommand),

#if REZ_FOX_VERSION<10304
	FXMAPFUNC(SEL_UPDATE,			FXWindow::ID_QUERY_TIP,		RezAction::onQueryTip)
#else
	FXMAPFUNC(SEL_QUERY_TIP,		0,				RezAction::onQueryTip)
#endif
};

FXIMPLEMENT(RezAction,FXMenuCommand,RezActionMap,ARRAYNUMBER(RezActionMap))
*/

RezAction::RezAction(AActionFactory *_actionFactory,const QIcon &ic) :
	QAction(NULL),
	/*
	FXMenuCommand(
		p,
...
RezAction::RezAction(const string untranslatedText,const QIcon &ic) :
	QAction(NULL),
...
RezAction::RezAction(QAbstractButton *abstractButton) :
	QAction(NULL),
	m_actionFactory(NULL),
...
const string RezAction::getStrippedUntranslatedText() const
{
	return stripElipsis(stripAmpersand(getUntranslatedText()));
}

const string RezAction::getStrippedText() const
{
