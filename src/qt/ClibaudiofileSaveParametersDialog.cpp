...
ClibaudiofileSaveParametersDialog::ClibaudiofileSaveParametersDialog(QWidget *mainWindow) :
	CActionParamDialog(mainWindow)
{
	...
	QWidget *p=newVertPanel(NULL);
		...
		CComboTextParamValue *sampleFormat=addComboTextEntry(p,N_("Sample Format"),sampleFormats,CActionParamDialog::cpvtAsInteger,"");
...
bool ClibaudiofileSaveParametersDialog::show(AFrontendHooks::libaudiofileSaveParameters &parameters,const string formatName)
{
	...
	{
		vector<string> compressionTypes;
		CComboTextParamValue *comboBox=getComboText("Compression Type");
...
void ClibaudiofileSaveParametersDialog::onSampleFormat_changed()
{
