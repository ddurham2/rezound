...
#ifndef __RezAction_H__
#define __RezAction_H__
...
class RezAction : public QAction
{
	Q_OBJECT
public:
	// when the action is triggered, the action factory will perform the action
    RezAction(AActionFactory *_actionFactory, const QIcon &ic=loadIcon("normal_action_buff.png"));

	// the action's task should be handled by something else
    RezAction(const string untranslatedText, const QIcon &ic=loadIcon("normal_action_buff.png"));
    ...
    RezAction(QAbstractButton *abstractButton);
