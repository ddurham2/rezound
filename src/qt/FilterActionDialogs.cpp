			...
			QWidget *p1=newHorzPanel(p0,false);
				...
				QWidget *p2=newVertPanel(p1,false);
					...
					QWidget *p3=newHorzPanel(p2,false);
						...
						QWidget *p4=newVertPanel(p3,false);
							addSlider(p4,
								N_("FK Rate"),
								"x",
	...
	ActionParamDialog(mainWindow)
{
	QWidget *p0=newHorzPanel(NULL);
		...
		QWidget *p1=newVertPanel(p0,false);
			addGraph(p1,
			...
			QWidget *p2=newHorzPanel(p1,false);
				...
				connect(numberOfOctaves,SIGNAL(changed()),this,SLOT(on_frequencyRange_changed()));
	...
	ActionParamDialog(mainWindow)
{
	...
	p0=newVertPanel(NULL);
		p1=newHorzPanel(p0,false);
			...
			p2=new QWidget();
			p2->setLayout(new QVBoxLayout);
			p2->layout()->setContentsMargins(0,0,0,0);
			p1->layout()->addWidget(p2);
				...
				b=new QToolButton;
				b->setText("->");
				b->setToolTip(_("Copy Response 1 to Response 2"));
				p2->layout()->addWidget(b);
				connect(b,SIGNAL(clicked()),this,SLOT(on_copy1To2()));

				b=new QToolButton;
				b->setText("<->");
				b->setToolTip(_("Swap Response 1 and Response 2"));
				p2->layout()->addWidget(b);
				connect(b,SIGNAL(clicked()),this,SLOT(on_swap1And2()));

				b=new QToolButton;
				b->setText("<-");
				b->setToolTip(_("Copy Response 2 to Response 1"));
				p2->layout()->addWidget(b);
				connect(b,SIGNAL(clicked()),this,SLOT(on_copy2To1()));
