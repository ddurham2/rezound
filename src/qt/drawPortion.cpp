...
static QColor axisColor(64,64,180,128); // alpha here
static QColor dBAxisColor(45,45,45,128); // alpha here

QColor backGroundColor(0,10,58);
static QColor selectedBackGroundColor(46,98,110);

static QColor waveformColor(255,255,255);
...
static QColor selectedWaveformColor(min(255, selectedBackGroundColor.red()+SELECTED_WAVE_ADD),min(255,selectedBackGroundColor.green()+SELECTED_WAVE_ADD),min(255,selectedBackGroundColor.blue()+SELECTED_WAVE_ADD)); // selectedBackGroundColor + SELECTED_WAVE_ADD
	
QColor clippedWaveformColor(255,0,127); // pink to stand out
 ...
 * horzZoomFactor:	how many sample positions are represented by one pixel.  hence, 1.0 is zoomed all the way in
 ...
 *
 */
void drawPortion(int left,int width,QPainter *dc,CSound *sound,int canvasWidth,int canvasHeight,int drawSelectStart,int drawSelectStop,double horzZoomFactor,sample_pos_t hOffset,float vertZoomFactor,int vOffset,bool darkened,bool invertColors)
{
...
// ??? 3 drawLines() calls might be better so we don't st the pen 15,000 times
						...
						// draw the background
						dc->setPen(invertColors ? invertColor(bgc,darkened) : bgc);
						dc->drawLine(x,channelTop,x,_min_y-1);
						dc->drawLine(x,_max_y+1,x,(int)round(channelTop+channelHeight));

						// draw the waveform
						dc->setPen(invertColors ? invertColor(wfc,darkened) : wfc);
						dc->drawLine(x,_min_y,x,_max_y);
					}
					else
					{
						dc->setPen(invertColors ? invertColor(backGroundColor,false) : backGroundColor);
				...
				// draw axis
				{
					dc->setPen(invertColors ? invertColor(axisColor,darkened) : axisColor);
					...
					dc->drawLine(left,(int)y,right+1,(int)y);
				}
		
				// draw -6dB lines
				{
					int y;
					dc->setPen(invertColors ? invertColor(dBAxisColor,darkened) : dBAxisColor);
		...
		else
		{
			...
			dc->fillRect(left,0,width,canvasHeight,backGroundColor);
		}
	}
	catch(exception &e)
	{
