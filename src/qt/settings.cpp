...
const QIcon loadIcon(const string baseFilename)
{
	return QIcon(loadPixmap(baseFilename));
}

const QPixmap loadPixmap(const string baseFilename)
{
	//printf("%s %d\n", (gSysDataDirectory+"/images/"+baseFilename).c_str(), QPixmap((gSysDataDirectory+"/images/"+baseFilename).c_str()).isNull());
	return QPixmap((gSysDataDirectory+"/images/"+baseFilename).c_str());
}
