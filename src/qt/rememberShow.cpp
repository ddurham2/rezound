...
bool rememberShow(QWidget *window,const string windowTitle)
{
	...
	const string title="Qt4" DOT "WindowDimensions" DOT windowTitle;
	...
	{
		int x=gSettingsRegistry->getValue<int>(title+"_X");
		int y=gSettingsRegistry->getValue<int>(title+"_Y");
		int width=gSettingsRegistry->getValue<int>(title+"_W");
		int height=gSettingsRegistry->getValue<int>(title+"_H");
...
#ifdef QT_RESTORE_WINDOW_POSITIONS
			// ??? UNTESTED CODE (probably is very wrong with multi heads)
		QDesktopWidget root;
		window->setGeometry(
			// make sure x,y are in visible range
			min(max(0,x),root.availableGeometry(root.screenNumber(window)).width()),
			min(max(0,y),root.availableGeometry(root.screenNumber(window)).height()),
...
void rememberHide(QWidget *window,const string windowTitle)
{
	const string title="Qt4" DOT "WindowDimensions" DOT windowTitle;
	//printf("closing window: %s %d %d\n",window->title().toStdString().c_str(),window->x(),window->y());
	gSettingsRegistry->setValue<int>(title+"_X",window->x()/*-decorWidth*/);
	gSettingsRegistry->setValue<int>(title+"_Y",window->y()/*-decorHeight*/);
	gSettingsRegistry->setValue<int>(title+"_W",window->width());
	gSettingsRegistry->setValue<int>(title+"_H",window->height());
}
