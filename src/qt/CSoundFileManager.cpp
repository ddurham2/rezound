...
{
	CSoundWindow *win=new CSoundWindow(mainWindow->getSoundWindowLayout(),loaded);

	soundWindows.push_back(win);

	// to avoid flicker, CSoundWindow is hidden initially, then layout is calculated, then setActiveState() shows it
...
void CSoundFileManager::setActiveSoundWindow(CSoundWindow *win)
{
	for(size_t t=0;t<soundWindows.size();t++)
	{
