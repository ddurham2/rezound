...
class FrequencyAnalyzerCanvas : public QWidget
{
public:
    FrequencyAnalyzerCanvas(QWidget *parent) :
		QWidget(parent),
		zoom(1.0),
		drawBarFreq(false)
	{
    ...
    virtual ~FrequencyAnalyzerCanvas()
	{
	}
...
FrequencyAnalyzer::FrequencyAnalyzer(QWidget *parent) :
	QWidget(parent),
...
void FrequencyAnalyzer::setSoundPlayer(ASoundPlayer *soundPlayer)
{
...
bool FrequencyAnalyzer::eventFilter(QObject *watched, QEvent *ev)
{
...
void FrequencyAnalyzer::timerEvent(QTimerEvent *e)
{
	if(
		e->timerId()==m_timerId && 
		m_soundPlayer!=NULL && 
...
void FrequencyAnalyzer::onZoomDial(int val)
{
	canvas->zoom=val;
	canvas->update();
}

void FrequencyAnalyzer::rebuildLabels(const vector<size_t> _frequencies,size_t octaveStride)
{
	...
	for(size_t t=0;t<canvas->analysis.size()/octaveStride;t++)
	{
		...
		// ??? should NOT be necessary, but for some reason it is.. bug? (also in LevelMeters.h)
